...
def initilize_right_side_functionality(app, df_full_data, df_lat_lon):
    ...
    def display_selected_data(district_list, chart_dropdown, year, avUK):
        ...
        #create an average of total UK data
        
        if chart_dropdown == "show_accidents_per_age":
            return accidents_per_age(dff, year, df_full_data, avUK)
        if chart_dropdown == "show_accidents_per_vehicle_age":
            return accidents_per_vehicle_age(dff, year, df_full_data, avUK)
        if chart_dropdown == "show_accidents_per_engine_capacity":
            return accidents_per_engine_capacity(dff, year, df_full_data, avUK)
        if chart_dropdown == "show_accidents_per_time":
            return accidents_per_time(dff, year, df_full_data, avUK)
        if chart_dropdown == "show_accidents_per_propulsion_code":
            return accidents_per_propulsion_code(dff, year, df_full_data, avUK)
        if chart_dropdown == "show_accidents_per_sex_of_driver":
            return accidents_per_sex_of_driver(dff, year, df_full_data, avUK)
        if chart_dropdown == "show_accidents_per_urban_or_rural_area":
            return accidents_per_urban_or_rural_area(dff, year, df_full_data, avUK)
        if chart_dropdown == "show_accidents_per_left_right_hand":
            return accidents_per_left_right_hand(dff, year, df_full_data, avUK)


def accidents_per_age(dff, year, df_full_data, avUK):
    """
...
def accidents_per_vehicle_age(dff, year, df_full_data, avUK):
    """
    This function generates the right side figure displaying accidents per age
    """
    if year == 2021:
...
def accidents_per_engine_capacity(dff, year, df_full_data, avUK):
    """
    ...
    if avUK == "yes":
        ...
        fig = px.histogram(rate_per_age_fulldata, x=AGGREGATE_BY, y="count", color=AGGREGATE_BY2, title=title, 
...
def accidents_per_time(dff, year, df_full_data, avUK):
    """
    ...
    #propulsion code
def accidents_per_propulsion_code(dff, year, df_full_data, avUK):
    """
    ...
    #sex driver DONE
def accidents_per_sex_of_driver(dff, year, df_full_data, avUK):
    """
    ...
    #urban or rural DONE
def accidents_per_urban_or_rural_area(dff, year, df_full_data, avUK):
    """
    ...
    #left or right hand
def accidents_per_left_right_hand(dff, year, df_full_data, avUK):
    """
